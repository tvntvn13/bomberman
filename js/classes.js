...
export class Player extends MovingElement {
  ...
  respawn() {
    ...
    this.x = respawn[0];
    this.y = respawn[1];
    document.getElementById(`block-${this.x}:${this.y}`).classList.add('player', 'invincible');
    this.alive = true;
    setTimeout(() => {
      this.invincible = false;
      document.getElementById(`block-${this.x}:${this.y}`).classList.remove('invincible');
    }, 3000);
  }
  clearAllBombs() {
    for (const bomb in bombsPlaced) {
      delete bombsPlaced[bomb];
    }
  }
}
...
export class Enemy extends MovingElement {
  ...
  move() {
    ...
    } else if (eval(this.allDirections[this.movementDirection])) {
      return;
    } else {
      let originalIndex = this.movementDirection;
      while (!eval(this.allDirections[this.movementDirection])) {
...
export class Bomb {
  ...
  explosionPrep() {
    ...
    for (let i = this.x + 1; i < map[this.y].length - 1 && i <= this.x + 3; i++) {
      let currentBlock = document.getElementById(`block-${i}:${this.y}`);
      if (currentBlock.classList.contains('solid-wall')) {
        break;
      } else if (currentBlock.classList.contains('softWall')) {
